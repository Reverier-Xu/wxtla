...
fn open_observed_raw_fixture_file_system(
  relative_path: &str,
) -> wxtla::Result<(wxtla::filesystems::fat::FatFileSystem, ByteSourceReadStats)> {
...
fn fat12_fixture_reads_regular_and_empty_files() {
  ...
  let testfile = file_system.open_content(&testfile_entry.node_id).unwrap();
  ...
  assert_eq!(
    file_system
      .open_content(&long_name_entry.node_id)
      .unwrap()
      .read_all()
      .unwrap(),
    Vec::<u8>::new()
