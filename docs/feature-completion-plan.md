...
- GPT probing and open now validate real headers while inferring additional block sizes up to `65536` in `src/volumes/gpt/mod.rs` and `src/volumes/gpt/parser.rs`
...
| Volume | GPT | Support is still strict to revision `1.0` despite broader header-size, block-size, and probe tolerance | `src/volumes/gpt/header.rs`, `src/volumes/gpt/constants.rs`, `src/volumes/gpt/mod.rs` | Some nonstandard GPT variants are still rejected | Expand revision compatibility only with real fixtures; do not loosen validation blindly |
