...
impl SevenZipArchive {
  ...
  fn entry_ref(&self, entry_id: &NamespaceNodeId) -> Result<&SevenZipEntry> {
    ...
    self
      .entries
      .get(index)
      .ok_or_else(|| Error::not_found(format!("missing 7z archive entry index: {index}")))
  }
}
...
impl Archive for SevenZipArchive {
  ...
  fn open_file(&self, entry_id: &NamespaceNodeId) -> Result<ByteSourceHandle> {
    ...
    let path = entry.extracted_path.as_ref().ok_or_else(|| {
      Error::invalid_format("7z file entry does not have a cached extraction path")
    })?;
...
fn build_tree(
  ...
  for path in &ordered_paths {
    let entry = builders
      .get(path)
      .ok_or_else(|| Error::invalid_format(format!("missing 7z entry builder for path: {path}")))?;
    let id = path_to_id.get(path).cloned().ok_or_else(|| {
      Error::invalid_format(format!("missing 7z entry identifier for path: {path}"))
    })?;
    entries.push(SevenZipEntry {
  ...
  for path in &ordered_paths {
    let child_id = path_to_id
      .get(path)
      .cloned()
      .ok_or_else(|| Error::invalid_format(format!("missing 7z path mapping for path: {path}")))?;
    ...
    let parent_index = match parent_path(path) {
      Some(parent) => entry_id_to_index(path_to_id.get(parent).ok_or_else(|| {
        Error::invalid_format(format!("missing 7z parent directory mapping: {parent}"))
      })?)?,
      None => 0,
    };
...
fn entry_id_to_index(entry_id: &NamespaceNodeId) -> Result<usize> {
  let bytes: [u8; 8] = entry_id
    .as_bytes()
    .try_into()
    .map_err(|_| Error::invalid_format("7z archive entry identifiers must be native u64 values"))?;
  usize::try_from(u64::from_le_bytes(bytes))
    .map_err(|_| Error::invalid_range("7z archive entry index is too large"))
}
