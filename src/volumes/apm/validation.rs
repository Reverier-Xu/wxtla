...
fn validate_descriptor(source_size: u64, descriptor: &ApmDriverDescriptor) -> Result<()> {
  ...
  let total_size = u64::from(descriptor.block_size)
    ...
    .ok_or_else(|| Error::invalid_range("apm device size overflow"))?;
    ...
    .map(|partition| {
      Ok((
        ...
        partition
          ...
          .span
          .end_offset()
          .ok_or_else(|| Error::invalid_range("apm partition end offset overflow"))?,
      ))
    })
...
fn validate_partition(
  ...
  let end_block = start_block
    ...
    .ok_or_else(|| Error::invalid_range("apm partition end block overflow"))?;
  ...
  let byte_offset = start_block
    ...
    .ok_or_else(|| Error::invalid_range("apm partition byte offset overflow"))?;
  let byte_size = block_count
    .checked_mul(u64::from(descriptor.block_size))
    .ok_or_else(|| Error::invalid_range("apm partition byte size overflow"))?;
  let byte_end = byte_offset
    .checked_add(byte_size)
    .ok_or_else(|| Error::invalid_range("apm partition byte end overflow"))?;
